...
export default function ProgramEditorPage() {
	...
	const handleSelectAll = () => {
		...
		// Only select implemented days from the current week
		const currentWeek = program.weeks.find((w) => w.weekNumber === currentWeek)
		if (!currentWeek) return

		const implementedDays = currentWeek.days
			.filter(
				(day) => day && !day.isRestDay && day.drills && day.drills.length > 0
			)
			.map((day) => `${currentWeek.weekNumber}-${day.dayNumber}`)
