...
function DraggableExercise({
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className='bg-gray-700/50 rounded p-2 cursor-grab active:cursor-grabbing border border-gray-600 hover:border-gray-500'
    >
      <div className='flex items-center gap-2'>
        <button
          {...attributes}
          {...listeners}
          className='p-1 rounded hover:bg-gray-600/50 text-gray-400 hover:text-gray-300 transition-colors'
        >
...
function DroppableDayContainer({
  ...
  return (
    <div
      ref={setNodeRef}
      className={`relative rounded-lg border transition-all duration-300 hover:shadow-lg min-h-[200px] ${
        isSelected
...
export default function WeekPlanner({
      ...
      case "paste":
        if (clipboard && clipboard.days && clipboard.days.length > 0) {
          ...
          if (clipboard.days.length > 1) {
            ...
            finalUpdatedDays.sort((a: any, b: any) => a.dayNumber - b.dayNumber)
  ...
  const handleDragStart = (event: DragStartEvent) => {
    ...
    for (const day of currentWeekData?.days || []) {
      const exercise = day.drills?.find((drill: any) => drill.id === exerciseId)
      if (exercise) {
    ...
    console.log("Drag start:", { exerciseId, foundExercise }) // Debug log
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    console.log("Drag end:", { activeId, overId }) // Debug log
    ...
    if (overId.startsWith("day-container-")) {
      ...
      for (const day of currentWeekData.days) {
        const exercise = day.drills?.find((drill: any) => drill.id === activeId)
        if (exercise) {
  ...
  return (
    <div className='space-y-6'>
      ...
      <div className='mb-6 text-sm text-gray-400 text-center'>
        💡 Tip: Press{" "}
        <kbd className='px-2 py-1 bg-gray-700 rounded text-xs'>Ctrl+A</kbd> to
        select all implemented days, then{" "}
        <kbd className='px-2 py-1 bg-gray-700 rounded text-xs'>Ctrl+V</kbd> to
        paste in order
      </div>
      ...
      <DragOverlay>
        {draggedExercise ? (
          <div className='bg-gray-700/50 rounded p-2 shadow-lg border border-gray-600'>
            <div className='flex items-center gap-2'>
