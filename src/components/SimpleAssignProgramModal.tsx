...
/** Used to avoid excessively-deep type inference from tRPC client list. */
interface ProgramAssignmentWithClient {
  id: string;
  programId: string;
  progress?: number;
...
export default function SimpleAssignProgramModal({
  ...
  // Sort clients alphabetically by name (cast to avoid deep tRPC type inference)
      ...
      ? (allClients as Client[]).flatMap(client =>
          (client.programAssignments ?? [])
            ...
            .map(assignment => ({
              ...
              progress: (assignment as { progress?: number }).progress,
              startDate: (assignment as { startDate?: Date | string | null })
                .startDate,
              assignedAt: (assignment as { assignedAt?: Date | string | null })
                .assignedAt,
              client: {
                id: client.id,
                name: client.name,
