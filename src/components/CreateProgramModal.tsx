...
export default function CreateProgramModal({
	...
	useEffect(() => {
		if (!initializedRef.current && watchedDuration) {
			const targetWeeksLength = watchedDuration || 4
			const initialWeeks = Array.from({ length: targetWeeksLength }, (_, i) => {
	...
	const removeWeek = (weekIndex: number) => {
		// Check if the week exists before trying to remove it
								...
								<div className="space-y-4">
									<div>
										...
										<Input
											...
											placeholder="e.g., Advanced Softball Training"
										/>
										{errors.title && (
